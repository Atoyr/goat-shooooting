...
Editorはビジュアル編集を標準モードとして提供します。左側からゲーム素材を選び、右側の日本語フォームで値を変更すると、Player、Enemy、Bullet、Weaponの見た目・軌道・弾幕が中央へ即時プレビューされます。Stageでは、中央キャンバス上のEnemyをドラッグして出現位置を変更し、下部タイムラインから出現イベントを選択できます。イベントの追加、複製、削除、出現時刻、編隊数、間隔も画面操作で設定できます。
